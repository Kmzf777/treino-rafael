...
const empurrarA = buscarDivisao('empurrarA')!
const exerciciosDeEmpurrarA = empurrarA.blocos.flatMap((b) => b.exercicios)

describe('ListaExercicios', () => {
  it('mostra os cabeçalhos de bloco', () => {
    render(<ListaExercicios divisao={empurrarA} onAbrir={() => {}} />)
    for (const bloco of empurrarA.blocos) {
      expect(screen.getByText(bloco.nome)).toBeInTheDocument()
    }
  })

  it('mostra os 9 exercícios com número e prescrição', () => {
    render(<ListaExercicios divisao={empurrarA} onAbrir={() => {}} />)
    expect(screen.getAllByRole('button')).toHaveLength(9)
    expect(screen.getByText('2.1')).toBeInTheDocument()
    expect(screen.getByText('4x5-8')).toBeInTheDocument()
  })

  it('avisa quando o exercício não tem clipe', () => {
    render(<ListaExercicios divisao={empurrarA} onAbrir={() => {}} />)
    expect(screen.getAllByText('sem clipe').length).toBe(
      exerciciosDeEmpurrarA.filter((e) => !e.recorte).length,
    )
  })
  ...
  it('marca os exercícios unilaterais', () => {
    ...
    expect(screen.getAllByText('unilateral').length).toBe(
      exerciciosDeEmpurrarA.filter((e) => e.unilateral).length,
    )
  })
  ...
  it('chama onAbrir com o id ao clicar', async () => {
    ...
    render(<ListaExercicios divisao={empurrarA} onAbrir={onAbrir} />)
    await usuario.click(screen.getAllByRole('button')[0])
    expect(onAbrir).toHaveBeenCalledWith('ea-agacha')
  })
